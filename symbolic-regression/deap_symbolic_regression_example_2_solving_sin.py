...
exp_function = lambda x: math.sin(x)
...
# Set our constants(terminals) and primatives(functions) set
...
pset.addEphemeralConstant("rand101", lambda: math.factorial(random.randint(0,7)))
...
f_points = [exp_function(x) for x in points]
plt.plot(points, f_points , label='expected function')

evl_func = toolbox.compile(expr=hof[0])
f_points  = [evl_func(x) for x in points]
plt.plot(points, f_points , label='evluated function')
plt.legend()
plt.plot()
